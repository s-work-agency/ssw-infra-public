...
- **규모**: 18개 모듈 / 5개 그룹 (도메인 서비스 · 플랫폼 · 클라이언트 · 공용 계약 라이브러리 · 운영 기반)
- **성격**: 특정 제품 전용이 아닌 **제품 불가지론(product-agnostic) 플랫폼**. 현재 이커머스 데모
  프로젝트가 첫 소비자로 붙어 라이브 운영 중입니다.
- **상태**: 엣지·인증·파일·미디어·알림·채팅에 관측성 3축과 설정·스케줄러까지 **스택 전체가
  쿠버네티스 클러스터에 올라 실사용 트래픽을 받고 있습니다.** 가장 최근에 착수한 원격 화면
  모듈만 구현·검증을 마치고 배포 대기 상태입니다.
...
flowchart TB
    ...
    SVC["<b>도메인 서비스 7</b><br/>auth · file · media · notification · chat · analytics<br/>+ remote(원격 화면 — 배포 대기)"]
...
| [서비스 카탈로그](docs/services.md) | 18개 모듈의 역할·기술 스택·현재 상태 |
